...
export function Footer({ variant = "dark" }: FooterProps) {
  ...
  const moonColor =
    variant === "light" ? "text-amber-300/40" : "text-emerald-400/60";
  const heartColor =
    variant === "light"
      ? "text-rose-300/60 fill-rose-300/60"
  ...
  return (
    <footer className="py-8 px-4 text-center space-y-3">
      <div className={`flex items-center justify-center gap-2 text-sm ${textColor}`}>
        <Moon className={`h-3.5 w-3.5 ${moonColor}`} />
        <span>Made with</span>
      ...
      <p className={`text-xs ${textColor}`}>
        &copy; {new Date().getFullYear()} Farhana Alam. All rights reserved.
      </p>
    </footer>
  );
}
